...
def print_parted_commands(device):
    ...
    os.remove('mkxboxfs.sh')

    f = open('mkxboxfs-linux.sh', 'w')
    f.write('#!/bin/bash\n')
    f.write('DEV={0}\n'.format(device))
    f.write('set -x -e\n')
    ...
    f.write('if ! which mkntfs; then echo "You need install NTFS-3g to mkntfs under linux/macOS"; exit 1; fi\n')
    f.write('sleep 1 # wait for reload gpt table')
    ...
    f.flush()
    f.close()
    os.chmod('mkxboxfs-linux.sh', 0o777)
    ...
    f.write('if ! which mkntfs; then\n')
    f.write('   echo "You need install NTFS-3g to mkntfs under linux/macOS";\n') 
    f.write('   exit 1\n')
    f.write('fi\n')
    f.write('sleep 1 # wait for reload gpt table')
    f.write('mkntfs -q "${DEV}s1" -f -L "Temp Content"\n')
    f.write('mkntfs -q "${DEV}s2" -f -L "User Content"\n')
    f.write('mkntfs -q "${DEV}s3" -f -L "System Support"\n')
    f.write('mkntfs -q "${DEV}s4" -f -L "System Update"\n')
    f.write('mkntfs -q "${DEV}s5" -f -L "System Update 2"\n')
    f.flush()
    f.close()
    os.chmod('mkxboxfs-mac.sh', 0o777)

    if sys.platform == 'linux2': 
        os.symlink('mkxboxfs-linux.sh', 'mkxboxfs.sh')
    elif sys.platform == 'darwin':
        os.symlink('mkxboxfs-mac.sh', 'mkxboxfs.sh')
    else:
